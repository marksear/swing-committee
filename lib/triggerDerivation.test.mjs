...
describe('deriveTriggerStopTarget — LONG', () => {
  it('produces expected zone / stop / target from known OHLC + ATR', () => {
    // lastClose=100, atrRaw=2 (LBR-aligned 0.35×ATR intraday formula):
    //   trigger_low  = 100
    //   trigger_high = 100 + 2 × 0.35 = 100.70
    //   stop         = 100 - 2 × 1.5  = 97
    //   R            = 100 - 97 = 3
    //   target       = 100 + 3 × 3 = 109
    // Was 0.5 (trigger_high=101) for swing-style; tightened 2026-04-29.
  ...
  it('rounds to default 0.01 tick size for equities (TXN-shape)', () => {
    ...
    //   trigger_low  = 261.41
    //   trigger_high = 261.41 + 4.27 × 0.35 = 262.9045 → 262.90
    ...
    // trigger_high near 262.905 (was 263.545 under 0.5×ATR)
    assert.ok(Math.abs(out.trigger_high - 262.905) < 0.011, `trigger_high ${out.trigger_high}`);
    // Stop unchanged (uses STOP_ATR_MULT=1.5, not the buffer multiplier).
    ...
    // Zone is now ~0.57% (was 0.82% under 0.5×ATR, 3% under fixed-3%):
    //   262.905 / 261.41 = 1.00572
    const bufferPct = (out.trigger_high - out.trigger_low) / out.trigger_low;
    assert.ok(bufferPct < 0.015, `buffer should be <1.5% on equity, got ${(bufferPct*100).toFixed(2)}%`);
  });
...
describe('deriveTriggerStopTarget — determinism', () => {
  ...
  it('input mutation does not affect prior output', () => {
    ...
    assert.equal(out1.trigger_high, 100.7);  // 100 + 0.35×2 (LBR intraday)
