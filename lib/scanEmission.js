 ...
 * @returns {{ scanRecord: object, shortlistEntries: object[] }}
 */
export function buildScanPayload({
  ...
  const shortlistEntries = signals
    .filter((s) => ['TAKE TRADE', 'DAY TRADE'].includes((s.verdict || '').toUpperCase()))
    .filter((s) => s.grade && GRADE_TO_RISK_PCT[s.grade] != null)
    .map((signal) => toShortlistEntry(signal, { scanId, accountSizeGbp, brokerMode, ruleSetVersion, createdAtUtc: scannedAtUtc }))
    .filter((entry) => entry != null);

  return { scanRecord, shortlistEntries };
}
...
function toShortlistEntry(signal, { scanId, accountSizeGbp, brokerMode, ruleSetVersion, createdAtUtc }) {
