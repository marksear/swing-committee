...
//
// Risk-pct policy (Mark 2026-04-29 — Task #52):
//   1% per trade is the standard across A+/A/B per
//   `feedback_small_account_sizing`. The earlier stepped ladder
//   (1.0/0.75/0.5) was a regression that diluted B-grade conviction
//   without basis in the desk reference. Top-3 trades/day × 1% = 3%
//   max open risk, well inside the 6% R5 cap.
...
// (which is DEMO-only by construction). Grade C also sized at 1% post-
// 2026-04-29 — the original 0.5% rationale (clear IG £0.10/pt floor)
// is satisfied at 1% too, and aligning C with the rest of the ladder
// keeps mechanics-testing representative of live sizing. The real C
// policy at the executor (per
// entry-rules/src/backtest/trade_management.risk_percent_for_grade) is
// still 0% — this table is the swing-committee-side mechanics-testing
// override only.
...
function toShortlistEntry(signal, ctx) {
  ...
  // ── Verdict consistency check (Task #65, 2026-04-29) ────────────────
  // If the stance text declares a VERDICT line that disagrees with
  // signal.verdict, the LLM produced inconsistent output and the row
  // is unsafe to ship. This is a defence-in-depth layer — the root
  // cause is fixed in buildTradeAnalysisText (which now honours the
  // actual verdict instead of hardcoding "TAKE TRADE"). The 2026-04-28
  // MS bug shipped with stance text saying TAKE TRADE but
  // extras.verdict=WATCHLIST; the operator saw what looked like a
  // tradeable signal that wasn't. Drop and emit a rejection so the
  // operator can audit.
  const declaredVerdict = (signal.verdict || '').toUpperCase().trim();
  const stanceVerdictMatch = /\*\*VERDICT:\*\*\s*([^\n—-]+)/i.exec(
    signal.rawSection || '',
  );
  if (stanceVerdictMatch && declaredVerdict) {
    const stanceVerdict = stanceVerdictMatch[1].toUpperCase().trim();
    if (stanceVerdict && stanceVerdict !== declaredVerdict) {
      emissionRejections.push({
        symbol: signal.ticker,
        direction,
        grade: signal.grade,
        reason: 'VERDICT_TEXT_VS_DECLARED_MISMATCH',
        stance_verdict: stanceVerdict,
  ...
  const targetPrice = parseNumber(signal.target);
  ...
  // ── Rule R2 — Reward:Risk surfacing (Task #53, 2026-04-29) ──────────
  // Per docs/specs/CANONICAL_ENTRY_RULES.md §R2 and the desk reference:
  //   (target − trigger_mid) / (trigger_mid − stop) ≥ 3.0   for LONG
  //   (trigger_mid − target) / (stop − trigger_mid) ≥ 3.0   for SHORT
  //
  // Phase 1 (today, no backtest): COMPUTE and SURFACE the R:R on every
  // entry as `extras.computed_reward_risk` plus a sub-3:1 boolean flag
  // `extras.r2_below_threshold`. Operator sees the number on every row
  // and can sort/filter sub-3:1 entries without blocking them.
  //
  // Phase 2 (deferred): hard-drop sub-3:1 entries. Requires a backtest
  // first per feedback_trading_discipline — the canonical policy is 3:1
  // but the new 0.5×ATR zone formula (Task #54) gets us ~2.2:1 in
  // typical conditions; flipping to hard-drop would gate most B-grade
  // candidates today. Decision to be made with proper R:R distribution
  // data, not a blunt threshold flip.
  //
  // Deliberately NOT pushed to emission_rejections — that sink is for
  // entries actually dropped, and a surfacing-only check shouldn't
  // contaminate it. The flag lives on extras instead.
  let rewardRisk = null;
  if (targetPrice != null && targetPrice > 0) {
    const triggerMid = (zone.low + zone.high) / 2;
    if (direction === 'LONG') {
      const risk = triggerMid - stopPrice;
      const reward = targetPrice - triggerMid;
      if (risk > 0) rewardRisk = reward / risk;
    } else {
      const risk = stopPrice - triggerMid;
      const reward = triggerMid - targetPrice;
      if (risk > 0) rewardRisk = reward / risk;
    }
  }
  // Stash on the signal so the extras: { ... } block downstream sees it.
  signal.__computedRewardRisk = rewardRisk;
  signal.__r2BelowThreshold = rewardRisk != null && rewardRisk < 3.0;
  ...
  return {
    ...
    symbol: signal.ticker,
    // Honour signal.market when route.js attached it (Task #66) — the
    // .L suffix has been stripped by then so the bare ticker can't tell.
    // Falls back to the .L heuristic for legacy callers / tests that
    // don't set signal.market explicitly.
    market: inferMarket({ ticker: signal.ticker, market: signal.market }),
    direction,
    ...
    extras: {
      ...
      // Server-computed R:R from zone/stop/target (Task #53). The legacy
      // `riskReward` field is the LLM-emitted text (e.g. "2.4:1"); this
      // is the deterministic numeric form, used for downstream gating
      // and audit. null when target is missing/invalid.
      computed_reward_risk: signal.__computedRewardRisk ?? null,
      // True when computed_reward_risk < 3.0 (canonical R2 threshold).
      // Phase 1 surfacing only — entry NOT dropped. See toShortlistEntry
      // for the Phase 2 hard-drop discussion.
      r2_below_threshold: signal.__r2BelowThreshold ?? false,
