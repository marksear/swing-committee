...
export function computeMclPolicy(factors, market = 'US') {
  ...
  // ── Volatility cap rule: if HIGH_VOL → regime = min(regime, YELLOW) ──
  // No GREEN allowed in high volatility. Vol is about tradeability not direction.
  // A +4 score in HIGH_VOL still gets YELLOW (with full sizing scaled by 0.75x).
  // RED stays RED (vol doesn't help a bearish environment).
  ...
  let longPillarDelta = 0
  let shortPillarDelta = 0
  ...
  // HIGH_VOL handling — per-side pillar deltas
  // Only tighten the DISFAVOURED side. The favoured side keeps its base threshold.
  // RED + HIGH_VOL: longs are disfavoured → tighten longs only, shorts stay easy
  // GREEN + HIGH_VOL: shorts are disfavoured → tighten shorts only (if ever needed)
  if (volatilityCapApplied && regime === 'RED') {
    longPillarDelta = +1   // Longs harder (disfavoured in RED)
    // shortPillarDelta stays 0 — shorts are favoured, don't penalize
  } else if (volatilityCapApplied && regime === 'GREEN') {
    shortPillarDelta = +1  // Shorts harder (disfavoured in GREEN)
    // longPillarDelta stays 0 — longs are favoured
  }
  ...
  const thresholds = {
    ...
    longPillars: clampPillars(base.longPillars + longPillarDelta),
    shortScore: clampScore(base.shortScore + shortScoreDelta),
    shortPillars: clampPillars(base.shortPillars + shortPillarDelta),
  }

  // ── Explainability string ──
  // Human-readable one-liner for diagnostics, logs, and UI tooltips
  const volTag = volatilityCapApplied ? ' HIGH_VOL' : ''
  const confTag = mclConfidence === 'LOW' ? ' LOW_CONF' : ''
  const explain = `MCL: ${regime}${volTag}${confTag} | score=${regimeScore > 0 ? '+' : ''}${regimeScore} | size L=${longSize} S=${shortSize} | gates: L≥${thresholds.longScore}% ${thresholds.longPillars}/6 S≥${thresholds.shortScore}% ${thresholds.shortPillars}/6`
  ...
  return {
    ...
    shortSize,
    thresholds,
    thresholdMods: { longScoreDelta, shortScoreDelta, longPillarDelta, shortPillarDelta },
    explain,
    weightedFactors,
    factorBreakdown: {
