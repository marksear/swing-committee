...
const TRADE_SIGNALS_HEADERS = ['Timestamp', 'Ticker', 'Direction', 'Entry', 'Stop', 'Target', 'Grade', 'Risk/Reward', 'Position Size', 'Pillars Assessment', 'Key Levels', 'Reasoning', 'Committee Stance'];
...
export async function appendTradeSignals(analysisResult) {
  ...
  for (const signal of signals) {
    ...
    rows.push([
      ...
      signal.riskReward || '',
      signal.positionSize || '',
      signal.tradeAnalysis?.pillarsAssessment || '',
      signal.tradeAnalysis?.keyLevels || '',
      signal.tradeAnalysis?.reasoning || '',
      analysisResult.committee_stance || ''
    ]);
  }
