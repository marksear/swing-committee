...
const SCAN_RESULTS_HEADERS = ['Timestamp', 'Direction', 'Ticker', 'Name', 'Price', 'Currency', 'Score', 'Reasoning'];
const TRADE_SIGNALS_HEADERS = ['Timestamp', 'Ticker', 'Direction', 'Entry', 'Stop', 'Target', 'Grade', 'Risk/Reward', 'Pillars', 'Setup/Reasoning', 'Committee Stance', 'Day Trade Tier', 'Day-1 Score', 'iATR', 'VWAP Bias'];
...
export async function appendTradeSignals(analysisResult) {
  ...
  console.log(`[Sheets] Processing ${signals.length} signals for Trade Signals sheet`);

  for (const signal of signals) {
    // Skip NO TRADE signals
    const verdict = (signal.verdict || signal.direction || '').toUpperCase();
    if (verdict.includes('NO TRADE') || verdict === 'PASS') {
      filteredCount++;
      console.log(`[Sheets] Filtered signal: ${signal.ticker || '?'} (verdict: ${verdict})`);
      continue;
    }
    ...
    // Build direction string — day trades include tier suffix
    const isDayTrade = signal.verdict === 'DAY TRADE' || signal.tier;
    let directionStr;
    if (isDayTrade && signal.tier) {
      const tierSuffix = signal.tier === 'A-GRADE' ? 'A' : 'B';
      directionStr = `DAY ${signal.direction || ''} (${tierSuffix})`.trim();
    } else if (isDayTrade) {
      directionStr = `DAY ${signal.direction || ''}`.trim();
    } else {
      directionStr = signal.direction || verdict || '';
    }
    ...
    rows.push([
      ...
      analysisResult.committeeStance || '',
      isDayTrade && signal.tier ? signal.tier : '—',
      isDayTrade && signal.totalScore != null ? `${signal.totalScore}/16` : '—',
      isDayTrade && signal.iATR != null ? Number(signal.iATR).toFixed(2) : '—',
      isDayTrade && signal.vwapBias ? signal.vwapBias : '—',
    ]);
  }

  console.log(`[Sheets] ${rows.length} signals to write, ${filteredCount} filtered out`);

  if (rows.length === 0) {
    console.log('[Sheets] No trade signals to append');
    return { success: true, rowsAdded: 0 };
  }
